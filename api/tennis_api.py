...
def _safe_key(key: str) -> str:
    """Masque la clé API pour les logs : affiche les 4 premiers caractères + ***"""
...
def _get(method: str, params: dict = {}) -> list | dict:
    ...
    try:
        ...
        if resp.status_code in (401, 403):
            _log("ERROR", "tennis_api", f"HTTP {resp.status_code} sur {method} — réponse : {resp.text[:300]}")
            return []
        ...
        _log("ERROR", "tennis_api", f"{method} — connexion impossible")
...
def fetch_player_utr(player_name: str) -> float:
    """Retourne le score UTR depuis la base locale (défaut : 10.0 si inconnu)."""
    try:
...
def fetch_atp_rankings() -> list[dict]:
    """Retourne le classement ATP (repli sur données statiques si l'API échoue)."""
    try:
