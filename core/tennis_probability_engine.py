...
def compute_match(
    utr_a, utr_b, elo_a, elo_b, surface="dur", sets=3,
    ...
    profil_a = profil(elo_a)
    profil_b = profil(elo_b)
    ...
    diff_utr = abs(utr_a - utr_b)
    if profil_a == "🧊 pressure" and profil_b == "🧊 pressure":
        volatility = "high"
    elif abs(proba_a - proba_b) < 0.05:
        volatility = SURFACES[surface]["volatilité_base"]
    else:
        volatility = "low"

    if sets == 5 and volatility == "medium":
        volatility = "high"
    elif sets == 3 and volatility == "high" and diff_utr > SEUIL_DIFF_UTR["équilibré"]:
        volatility = "medium"

    if type_tournoi in TOURNOIS:
        volatility = TOURNOIS[type_tournoi]["volatilité"]
    ...
    if cote_a and cote_b:
        book_a = 1 / cote_a
        book_b = 1 / cote_b
        edge_a = proba_a - book_a
        edge_b = proba_b - book_b
        if edge_a > 0.02:
            bet_type, bet_side = "winner", "A"
        elif edge_b > 0.02:
            bet_type, bet_side = "winner", "B"
        else:
            prob_over = (high - line) / (high - low)
            prob_under = 1 - prob_over
            if volatility == "high":
                if prob_over > 0.57:
                    bet_type, bet_side = "games", "over"
                elif prob_under > 0.57:
                    bet_type, bet_side = "games", "under"
        if bet_type != "none":
            stake = min(3, max(0.8, 2 if abs(edge_a if bet_side == "A" else edge_b) > 0.05 else 1.2))

    return {
        "proba_a": round(proba_a * 100, 1),
        "proba_b": round(proba_b * 100, 1),
